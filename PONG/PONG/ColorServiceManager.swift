...
class ColorServiceManager : NSObject {
    ...
    func send(positionX: Int8) {
        NSLog("%@", "sendColor: \(positionX) to \(session.connectedPeers.count) peers")
        ...
        if session.connectedPeers.count > 0 {
            do {
                ...
                let data = withUnsafePointer(to: &value) {
                    Data(bytes: UnsafePointer($0), count: MemoryLayout.size(ofValue: positionX))
                }
...
extension ColorServiceManager : MCSessionDelegate {
    ...
    func session(_ session: MCSession, didReceive data: Data, fromPeer peerID: MCPeerID) {
        ...
        self.delegate?.colorChanged(manager: self, colorString: Int8(bitPattern: num))
    }
